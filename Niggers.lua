...
-- === SIDE MENU (MOBILE BUTTONS) — 45% smaller ===
...
-- Original size was 180x210, reduced by 45%: 180*0.55=99, 210*0.55=115.5
menuFrame.Size = UDim2.new(0, 99, 0, 116); menuFrame.Position = UDim2.new(1, -109, 0.1, 0); menuFrame.BackgroundTransparency = 1

local menuLayout = Instance.new("UIListLayout", menuFrame)
menuLayout.SortOrder = Enum.SortOrder.LayoutOrder; menuLayout.Padding = UDim.new(0, 5)

local function createSideButton(text, order, callback)
    local btn = Instance.new("TextButton", menuFrame)
    -- Original height was 45, reduced by 45%: 45*0.55=24.75
    btn.Size = UDim2.new(1, 0, 0, 25); btn.BackgroundColor3 = Color3.fromRGB(0, 25, 70)
    btn.BorderSizePixel = 0; btn.LayoutOrder = order; btn.Text = "  "..text
    btn.TextColor3 = Color3.new(1, 1, 1); btn.Font = Enum.Font.GothamBold; btn.TextSize = 7; btn.TextXAlignment = Enum.TextXAlignment.Left
    ...
    -- Original indicator 10x10, reduced: ~5x5
    indicator.Size = UDim2.new(0, 6, 0, 6); indicator.Position = UDim2.new(1, -11, 0.5, -3)
...
-- ==========================================================
-- === ASSER HELPER UI (with Auto Play Version section inside)
-- ==========================================================
local speedGui = Instance.new("ScreenGui", CoreGui)
speedGui.Name = "SpeedConfigGui"; speedGui.ResetOnSpawn = false

-- CHANGE 1: Removed "Duels TP" divider label, moved AP version in.
-- New full height: original 260 + AP version section (~90px) = 350
local HELPER_FULL_H = 350
...
-- CHANGE 2: Duels TP button directly, NO divider label above it
local duelsTpBtn = Instance.new("TextButton", speedMain)
duelsTpBtn.Size = UDim2.new(0, 160, 0, 34); duelsTpBtn.Position = UDim2.new(0, 10, 0, 178)
...
-- CHANGE 3: Auto Play Version section embedded inside Asser Helper
local apDivider = Instance.new("TextLabel", speedMain)
apDivider.Size = UDim2.new(1, -20, 0, 20); apDivider.Position = UDim2.new(0, 10, 0, 222)
apDivider.BackgroundTransparency = 1; apDivider.Text = "— Auto Play Version —"
apDivider.TextColor3 = Color3.fromRGB(0, 188, 255); apDivider.Font = Enum.Font.GothamBold; apDivider.TextSize = 10

local function makeApBtnInHelper(label, posY)
    local b = Instance.new("TextButton", speedMain)
    b.Size = UDim2.new(0, 160, 0, 32); b.Position = UDim2.new(0, 10, 0, posY)
...
local apV1Btn = makeApBtnInHelper("Auto Play v1  ✔", 248)
local apV2Btn = makeApBtnInHelper("Auto Play v2",    288)
