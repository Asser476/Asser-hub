...
-- === MOBILE BUTTON SCALE ===
-- Default is 25% bigger than the 45%-smaller base. Base = 99w, 25px h.
-- 25% bigger: width=124, height=31
local MOBILE_DEFAULT_SCALE = 1.25  -- represents "50%" on slider (the default 25% bigger)
local mobileScaleFactor = MOBILE_DEFAULT_SCALE

local BASE_BTN_W = 99
local BASE_BTN_H = 25
local BASE_FRAME_W = 99
local BASE_FONT_SIZE = 7
local BASE_INDICATOR = 6
local BASE_PADDING = 5

local sideButtonRefs = {}  -- stores {btn, indicator, text} for rescaling

local function applyMobileScale(scale)
    mobileScaleFactor = scale
    local w  = math.floor(BASE_FRAME_W * scale)
    local h  = math.floor(BASE_BTN_H   * scale)
    local ind = math.max(4, math.floor(BASE_INDICATOR * scale))
    local fs  = math.max(6, math.floor(BASE_FONT_SIZE  * scale))
    local frameH = 0
    for _, ref in ipairs(sideButtonRefs) do
        ref.btn.Size = UDim2.new(1, 0, 0, h)
        ref.btn.TextSize = fs
        ref.indicator.Size = UDim2.new(0, ind, 0, ind)
        ref.indicator.Position = UDim2.new(1, -(ind + 5), 0.5, -math.floor(ind/2))
        frameH = frameH + h + BASE_PADDING
    end
    if menuFrame then
        menuFrame.Size = UDim2.new(0, w, 0, frameH)
        menuFrame.Position = UDim2.new(1, -(w + 10), 0.1, 0)
    end
end
...
local animatorsCache = setmetatable({}, {__mode = "k"})
local optLoopConn
local optDescAddedConn

local function regAnimatorOpt(a)
    if not a or animatorsCache[a] then return end
    animatorsCache[a] = true
    for _,t in pairs(a:GetPlayingAnimationTracks()) do t:Stop(0) end
    a.AnimationPlayed:Connect(function(t) if optLoopConn then t:Stop(0) end end)
end
    ...
    if state then
        ...
        for _,v in pairs(Workspace:GetDescendants()) do
            ...
            elseif v:IsA("BasePart") then
                originalParts[v] = {Material = v.Material, Reflectance = v.Reflectance, CastShadow = v.CastShadow}
        ...
        optDescAddedConn = Workspace.DescendantAdded:Connect(function(v)
            if v:IsA("Animator") then regAnimatorOpt(v) end
        end)
        optLoopConn = RunService.RenderStepped:Connect(function()
            for a in pairs(animatorsCache) do
                if a and a.Parent then
                    for _,t in pairs(a:GetPlayingAnimationTracks()) do t:Stop(0) end
                else
                    animatorsCache[a] = nil
                end
            end
        end)
    else
        ...
        for v, data in pairs(originalParts) do
        ...
        animatorsCache = setmetatable({}, {__mode = "k"})
        settings().Rendering.QualityLevel = Enum.QualityLevel.Automatic
        if optDescAddedConn then optDescAddedConn:Disconnect() optDescAddedConn = nil end
        if optLoopConn then optLoopConn:Disconnect() optLoopConn = nil end
    end
end
...
-- ==========================================================
-- === SIDE MENU (MOBILE BUTTONS) — default 25% bigger than base
-- ==========================================================
...
menuLayout.SortOrder = Enum.SortOrder.LayoutOrder; menuLayout.Padding = UDim.new(0, BASE_PADDING)

local function createSideButton(text, order, callback)
    local btn = Instance.new("TextButton", menuFrame)
    btn.Size = UDim2.new(1, 0, 0, BASE_BTN_H)
    ...
    btn.TextSize = BASE_FONT_SIZE; btn.TextXAlignment = Enum.TextXAlignment.Left
    Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 6)
    local stroke = Instance.new("UIStroke", btn)
    stroke.Thickness = 1; stroke.Color = Color3.fromRGB(0, 188, 255); stroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    local indicator = Instance.new("Frame", btn)
    indicator.Size = UDim2.new(0, BASE_INDICATOR, 0, BASE_INDICATOR)
    indicator.Position = UDim2.new(1, -(BASE_INDICATOR + 5), 0.5, -math.floor(BASE_INDICATOR/2))
    ...
    table.insert(sideButtonRefs, {btn = btn, indicator = indicator, text = text})
...
-- Apply default 25% bigger scale immediately after buttons are created
applyMobileScale(MOBILE_DEFAULT_SCALE)
...
-- Important: disable scroll grabbing while slider is held
...
local function makeToggleBtn(txt, callback)
    ...
    frame.Size = UDim2.new(1, -5, 0, 38); frame.BackgroundColor3 = Color3.fromRGB(0, 35, 95); frame.BorderSizePixel = 0
    ...
    toggle.Size = UDim2.new(0, 30, 0, 18); toggle.Position = UDim2.new(1, -40, 0, 10); toggle.BackgroundColor3 = Color3.fromRGB(0, 15, 50); toggle.Text = ""
    Instance.new("UICorner", toggle).CornerRadius = UDim.new(1, 0)
    local circle = Instance.new("Frame", toggle)
    circle.Size = UDim2.new(0, 12, 0, 12); circle.Position = UDim2.new(0, 3, 0, 3); circle.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    ...
    local function updateVisual(active)
        ...
        TweenService:Create(circle, TweenInfo.new(0.2), {Position = active and UDim2.new(1, -15, 0, 3) or UDim2.new(0, 3, 0, 3)}):Play()
    end
...
toggles["Optimizer"]                = makeToggleBtn("Optimizer",                function(v) toggleOptimizer(v) end)

-- ==========================================================
-- === MOBILE BUTTONS SIZE SLIDER (inside main scroll)
-- ==========================================================
-- Slider range: 50% to 100% displayed, mapped to scale 1.25 (default) to 2.5 (max)
-- 50% shown = MOBILE_DEFAULT_SCALE (1.25), 100% = 2.5
local SLIDER_MIN_SCALE = 1.25
local SLIDER_MAX_SCALE = 2.5
local sliderCurrentPct = 50  -- starts at 50% (default)
local sliderDragging = false

local sliderFrame = Instance.new("Frame", scroll)
sliderFrame.Size = UDim2.new(1, -5, 0, 70); sliderFrame.BackgroundColor3 = Color3.fromRGB(0, 35, 95); sliderFrame.BorderSizePixel = 0
Instance.new("UICorner", sliderFrame).CornerRadius = UDim.new(0, 8)

local sliderTitleLbl = Instance.new("TextLabel", sliderFrame)
sliderTitleLbl.Size = UDim2.new(1, -10, 0, 20); sliderTitleLbl.Position = UDim2.new(0, 8, 0, 4)
sliderTitleLbl.BackgroundTransparency = 1; sliderTitleLbl.TextColor3 = Color3.fromRGB(255, 255, 255)
sliderTitleLbl.Font = Enum.Font.GothamMedium; sliderTitleLbl.TextSize = 10
sliderTitleLbl.TextXAlignment = Enum.TextXAlignment.Left
sliderTitleLbl.Text = "Mobile Buttons Size:  50%"

local sliderTrack = Instance.new("Frame", sliderFrame)
sliderTrack.Size = UDim2.new(1, -20, 0, 8); sliderTrack.Position = UDim2.new(0, 10, 0, 28)
sliderTrack.BackgroundColor3 = Color3.fromRGB(0, 15, 50); sliderTrack.BorderSizePixel = 0
Instance.new("UICorner", sliderTrack).CornerRadius = UDim.new(1, 0)

local sliderFill = Instance.new("Frame", sliderTrack)
sliderFill.Size = UDim2.new(0, 0, 1, 0); sliderFill.Position = UDim2.new(0, 0, 0, 0)
sliderFill.BackgroundColor3 = Color3.fromRGB(0, 188, 255); sliderFill.BorderSizePixel = 0
...
sliderKnob.BackgroundColor3 = Color3.fromRGB(0, 188, 255); sliderKnob.BorderSizePixel = 0
Instance.new("UICorner", sliderKnob).CornerRadius = UDim.new(1, 0)
Instance.new("UIStroke", sliderKnob).Color = Color3.fromRGB(255, 255, 255)

-- Reset button
local resetBtn = Instance.new("TextButton", sliderFrame)
resetBtn.Size = UDim2.new(1, -20, 0, 22); resetBtn.Position = UDim2.new(0, 10, 0, 42)
resetBtn.BackgroundColor3 = Color3.fromRGB(0, 80, 180); resetBtn.Text = "Reset to Default"
resetBtn.TextColor3 = Color3.new(1, 1, 1); resetBtn.Font = Enum.Font.GothamBold; resetBtn.TextSize = 10
resetBtn.BorderSizePixel = 0
Instance.new("UICorner", resetBtn).CornerRadius = UDim.new(0, 6)
Instance.new("UIStroke", resetBtn).Color = Color3.fromRGB(0, 220, 255)

local function updateSliderVisual(pct)
    -- pct is 0..100 representing 50%..100% display
    sliderCurrentPct = math.clamp(pct, 0, 100)
    local displayPct = math.floor(50 + sliderCurrentPct * 0.5)  -- 50% to 100%
    sliderTitleLbl.Text = "Mobile Buttons Size:  " .. displayPct .. "%"
    sliderFill.Size = UDim2.new(sliderCurrentPct / 100, 0, 1, 0)
    sliderKnob.Position = UDim2.new(sliderCurrentPct / 100, -8, 0.5, -8)
    local scale = SLIDER_MIN_SCALE + (sliderCurrentPct / 100) * (SLIDER_MAX_SCALE - SLIDER_MIN_SCALE)
    applyMobileScale(scale)
end

-- Initialize slider at 50% (default)
updateSliderVisual(0)

local function getSliderPct(inputPos)
    local trackPos = sliderTrack.AbsolutePosition
    local trackSize = sliderTrack.AbsoluteSize
    local relX = inputPos.X - trackPos.X
    return math.clamp(relX / trackSize.X, 0, 1) * 100
end

sliderTrack.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        sliderDragging = true
        scroll.ScrollingEnabled = false  -- disable scroll grab while dragging slider
        updateSliderVisual(getSliderPct(input.Position))
    end
end)
...
UIS.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        if sliderDragging then
            ...
            scroll.ScrollingEnabled = true  -- re-enable scrolling after release
        end
    end
end)

resetBtn.MouseButton1Click:Connect(function()
    updateSliderVisual(0)  -- back to 50% display = default scale
    resetBtn.Text = "Reset!"
    resetBtn.BackgroundColor3 = Color3.fromRGB(0, 160, 80)
    task.delay(1, function()
        resetBtn.Text = "Reset to Default"
        resetBtn.BackgroundColor3 = Color3.fromRGB(0, 80, 180)
    end)
end)
...
local HELPER_FULL_H = 333  -- 5% shorter than 350 (350 * 0.95 = 332.5)
...
speedTitle.Size = UDim2.new(1, 0, 0, 28); speedTitle.BackgroundColor3 = Color3.fromRGB(0, 45, 120)
...
expandBtn.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(speedMain, TweenInfo.new(0.2), {Size = UDim2.new(0, 180, 0, isShrunk and 28 or HELPER_FULL_H)}):Play()
...
local function createSpeedBox(label, defaultVal, maxAllowed, posY, callback)
    ...
    lbl2.Size = UDim2.new(0, 100, 0, 28); lbl2.Position = UDim2.new(0, 10, 0, posY); lbl2.Text = label
    ...
    box.Size = UDim2.new(0, 50, 0, 22); box.Position = UDim2.new(1, -60, 0, posY + 3)
...
createSpeedBox("Speed Going:",   57, 61,  33, function(v) SPEED_IDA   = v end)
createSpeedBox("Speed Coming:",  29, 31,  64, function(v) SPEED_VOLTA = v end)
createSpeedBox("Lock in Speed:", 57, 61,  95, function(v) floatSpeed  = v end)

local floatActionBtn = Instance.new("TextButton", speedMain)
floatActionBtn.Size = UDim2.new(0, 160, 0, 28); floatActionBtn.Position = UDim2.new(0, 10, 0, 126)
...
duelsTpBtn.Size = UDim2.new(0, 160, 0, 30); duelsTpBtn.Position = UDim2.new(0, 10, 0, 163)
...
-- Auto Play Version buttons inside helper (no divider label)
local function makeApBtnInHelper(label, posY)
    local b = Instance.new("TextButton", speedMain)
    b.Size = UDim2.new(0, 160, 0, 28); b.Position = UDim2.new(0, 10, 0, posY)
...
local apV1Btn = makeApBtnInHelper("Auto Play v1  ✔", 202)
local apV2Btn = makeApBtnInHelper("Auto Play v2",    238)
