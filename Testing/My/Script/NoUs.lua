...
local function cleanupOldGUI()
    local guis = {"AsserDuels", "DropToggleGui", "MobileButtonsGui", "MenuToggleButtonGui", "SpeedConfigGui", "AsserSideMenuGui", "AsserNotifyGui", "RagdollTutorGui", "AsserAutoPlayGui", "StatsDisplay"}
...
local function makeDraggable(guiObj)
    ...
    UIS.InputChanged:Connect(function(input)
        if isSliderDragging then dragging = false return end
...
-- === SIZE SLIDER FACTORY ===
-- Returns a frame containing a size slider (0-100%), calls onChanged(scale) when changed
-- scale: 0.5 to 1.5 mapped to 0-100%, default 50% = 1.0 scale
local function makeScaleSlider(parent, posY, defaultPct, onChanged)
    local SLIDER_W = 160
    local sliderFrame = Instance.new("Frame", parent)
    sliderFrame.Size = UDim2.new(0, SLIDER_W, 0, 28)
    sliderFrame.Position = UDim2.new(0, 10, 0, posY)
    sliderFrame.BackgroundTransparency = 1

    local sliderLbl = Instance.new("TextLabel", sliderFrame)
    sliderLbl.Size = UDim2.new(0, 80, 0, 14)
    sliderLbl.Position = UDim2.new(0, 0, 0, 0)
    sliderLbl.BackgroundTransparency = 1
    sliderLbl.Text = "UI Size: "..tostring(math.floor(defaultPct*100)).."%"
    sliderLbl.TextColor3 = Color3.new(1,1,1)
    sliderLbl.Font = Enum.Font.GothamMedium
    sliderLbl.TextSize = 9
    sliderLbl.TextXAlignment = Enum.TextXAlignment.Left

    local track = Instance.new("Frame", sliderFrame)
    track.Size = UDim2.new(0, SLIDER_W, 0, 6)
    track.Position = UDim2.new(0, 0, 0, 18)
    track.BackgroundColor3 = Color3.fromRGB(0, 15, 50)
    track.BorderSizePixel = 0
    Instance.new("UICorner", track).CornerRadius = UDim.new(1, 0)
    Instance.new("UIStroke", track).Color = Color3.fromRGB(0, 188, 255)
    ...
    knob.Position = UDim2.new(defaultPct, -8, 0.5, -8)
    knob.BackgroundColor3 = Color3.fromRGB(0, 188, 255)
    knob.Text = ""
    knob.BorderSizePixel = 0
    Instance.new("UICorner", knob).CornerRadius = UDim.new(1, 0)

    local draggingSlider = false
    local currentPct = defaultPct
    ...
    local function updateSlider(pct)
        ...
        sliderLbl.Text = "UI Size: "..tostring(math.floor(pct*100)).."%"
        -- map pct 0->1 to scale 0.5->1.5
        local scale = 0.5 + pct
        onChanged(scale)
    end
    ...
    UIS.InputChanged:Connect(function(input)
        if draggingSlider and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            ...
            local relX = input.Position.X - trackPos
            updateSlider(relX / trackW)
        end
    end)
    ...
    UIS.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            if draggingSlider then
                ...
                task.defer(function() isSliderDragging = false end)
            end
        end
    end)

    return sliderFrame, function(pct) updateSlider(pct) end
end
...
-- === MOBILE BUTTON POSITION SAVE/LOAD ===
local mobilePosFolder = "AsserDuels"
local mobilePosFile   = mobilePosFolder .. "/mobilepos.json"
local savedMobilePositions = {}

local function loadMobilePositions()
    if isfile and isfile(mobilePosFile) then
        local ok, data = pcall(function() return HttpService:JSONDecode(readfile(mobilePosFile)) end)
        if ok and type(data) == "table" then
            savedMobilePositions = data
        end
    end
end

local function saveMobilePositions()
    if writefile then
        if not isfolder(mobilePosFolder) then makefolder(mobilePosFolder) end
        writefile(mobilePosFile, HttpService:JSONEncode(savedMobilePositions))
    end
end
...
-- === OPTIMIZER / ANTI-LAG LOGIC ===
local animators = {}
local animLoop = false

local function regAnimator(a)
    if not a or animators[a] then return end
    animators[a] = true
    for _,t in pairs(a:GetPlayingAnimationTracks()) do t:Stop(0) end
    a.AnimationPlayed:Connect(function(t) t:Stop(0) end)
end

for _,v in pairs(Workspace:GetDescendants()) do
    if v:IsA("Animator") then regAnimator(v) end
end
...
local function applyOptimizer(state)
    if state then
        ...
        animLoop = true
        showNotification("Optimizer Applied!")
    else
        animLoop = false
    end
end
...
local function toggleFloat(state)
    ...
    if floaton then
        ...
        startFloat()
        -- Start aimbot when Lock in activates
        ...
        stopFloat()
        -- Stop aimbot when Lock in deactivates
...
dropGui.Name = "DropToggleGui"
dropGui.Enabled = false
dropGui.ResetOnSpawn = false

local dropToggleBtn = Instance.new("TextButton", dropGui)
dropToggleBtn.Name = "ToggleButton"
dropToggleBtn.BackgroundColor3 = Color3.fromRGB(255, 60, 60)
dropToggleBtn.Position = UDim2.new(0.05, 0, 0.4, 0)
dropToggleBtn.Size = UDim2.new(0, 120, 0, 40)
dropToggleBtn.Font = Enum.Font.GothamBold
dropToggleBtn.Text = "DROP: OFF"
dropToggleBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
dropToggleBtn.TextSize = 14
makeDraggable(dropToggleBtn)
Instance.new("UICorner", dropToggleBtn).CornerRadius = UDim.new(0, 8)

dropToggleBtn.MouseButton1Click:Connect(function()
    local newState = not dropEnabled
    toggleDrop(newState)
    if newState then
        dropToggleBtn.Text = "DROP: ON"
        dropToggleBtn.BackgroundColor3 = Color3.fromRGB(60, 255, 60)
    else
        dropToggleBtn.Text = "DROP: OFF"
        dropToggleBtn.BackgroundColor3 = Color3.fromRGB(255, 60, 60)
    end
...
-- ==========================================================
-- === SIDE MENU (MOBILE BUTTONS) with Lock System + Slider
-- ==========================================================
...
-- Container that holds everything in the side menu (for scaling)
local sideContainer = Instance.new("Frame", sideGui)
sideContainer.Size = UDim2.new(0, 210, 0, 330)
sideContainer.Position = UDim2.new(1, -220, 0.05, 0)
sideContainer.BackgroundTransparency = 1
sideContainer.Name = "SideContainer"

local sideScale = Instance.new("UIScale", sideContainer)
sideScale.Scale = 1.0

-- We'll store button data for locking/dragging
local sideButtonData = {}

local menuLayout = Instance.new("UIListLayout", sideContainer)
menuLayout.SortOrder = Enum.SortOrder.LayoutOrder
menuLayout.Padding = UDim.new(0, 8)

-- draggable connection store per button
local btnDragConns = {}

local function makeBtnDraggable(btn, key)
    -- Remove existing connections
    if btnDragConns[key] then
        for _, c in ipairs(btnDragConns[key]) do c:Disconnect() end
    end
    btnDragConns[key] = {}
    ...
    local c2 = UIS.InputChanged:Connect(function(input)
        ...
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            ...
            -- Save position
            savedMobilePositions[key] = {x = btn.Position.X.Offset, y = btn.Position.Y.Offset, xs = btn.Position.X.Scale, ys = btn.Position.Y.Scale}
        end
    end)
    table.insert(btnDragConns[key], c1)
    table.insert(btnDragConns[key], c2)
end

local function removeBtnDraggable(key)
    if btnDragConns[key] then
        for _, c in ipairs(btnDragConns[key]) do c:Disconnect() end
        btnDragConns[key] = {}
    end
...
local function createSideButton(text, order, callback)
    -- Wrapper frame holds lock + button together in list layout
    local wrapper = Instance.new("Frame", sideContainer)
    wrapper.Size = UDim2.new(1, 0, 0, 45)
    wrapper.BackgroundTransparency = 1
    wrapper.LayoutOrder = order
    wrapper.Name = "Wrapper_"..text

    local lockBtn = Instance.new("TextButton", wrapper)
    lockBtn.Size = UDim2.new(0, 30, 0, 30)
    lockBtn.Position = UDim2.new(0, 0, 0.5, -15)
    lockBtn.BackgroundColor3 = Color3.fromRGB(0, 35, 95)
    ...
    btn.Size = UDim2.new(1, -38, 0, 45)
    btn.Position = UDim2.new(0, 36, 0, 0)
    btn.BackgroundColor3 = Color3.fromRGB(0, 25, 70)
    ...
    local savedPos = savedMobilePositions[text]
    if savedPos then
        btn.Position = UDim2.new(savedPos.xs or 0, savedPos.x or 36, savedPos.ys or 0, savedPos.y or 0)
    end

    local isLocked = true  -- starts locked (no drag)
    sideButtonData[text] = {btn = btn, locked = true, wrapper = wrapper}

    lockBtn.MouseButton1Click:Connect(function()
        isLocked = not isLocked
        sideButtonData[text].locked = isLocked
        if isLocked then
            lockBtn.Text = "🔒"
            removeBtnDraggable(text)
            -- Snap back into wrapper layout position
            btn.Position = UDim2.new(0, 36, 0, 0)
            savedMobilePositions[text] = nil
        else
            lockBtn.Text = "🔓"
            makeBtnDraggable(btn, text)
        end
    end)
...
local dropBtnSide, dropIndicator = createSideButton("Drop", 4, function(v)
    cancelAutoPlays()
    if floaton then
        toggleFloat(false)
    ...
    toggleDrop(v)
    if toggles["Drop Button"] then toggles["Drop Button"].update(v) end
    if v then
        dropToggleBtn.Text = "DROP: ON"
        dropToggleBtn.BackgroundColor3 = Color3.fromRGB(60, 255, 60)
    else
...
-- Size slider for mobile buttons
local sideSizeSlider, setSideSizeSlider = makeScaleSlider(sideContainer, 0, 0.5, function(scale)
    sideScale.Scale = scale
end)
sideSizeSlider.LayoutOrder = 99
sideSizeSlider.Size = UDim2.new(1, -10, 0, 32)
sideSizeSlider.Position = UDim2.new(0, 5, 0, 0)
...
scroll.Size = UDim2.new(1, -20, 1, -115); scroll.Position = UDim2.new(0, 10, 0, 45)
...
toggles["Optimizer"]                = makeToggleBtn("Optimizer",                function(v) applyOptimizer(v) end)
toggles["Unwalk Animation"]         = makeToggleBtn("Unwalk Animation",         function(v) if v then enableUnwalk() else disableUnwalk() end end)

-- Main UI size slider
local mainSliderFrame, setMainSlider = makeScaleSlider(main, 0, 0.5, function(scale)
    mainScale.Scale = scale
end)
mainSliderFrame.Position = UDim2.new(0, 10, 1, -68)
mainSliderFrame.Size = UDim2.new(1, -20, 0, 28)

local botBar = Instance.new("Frame", main)
botBar.Size = UDim2.new(1, 0, 0, 36); botBar.Position = UDim2.new(0, 0, 1, -36); botBar.BackgroundTransparency = 1
local saveBtn = Instance.new("TextButton", botBar)
saveBtn.Size = UDim2.new(1, -20, 0, 26); saveBtn.Position = UDim2.new(0, 10, 0.5, -13)
...
local helperScale = Instance.new("UIScale", speedMain)
helperScale.Scale = 1.0
...
-- Auto Play Version inside Helper
local apDivider = Instance.new("TextLabel", speedMain)
apDivider.Size = UDim2.new(1, -20, 0, 18); apDivider.Position = UDim2.new(0, 10, 0, 172)
apDivider.BackgroundTransparency = 1; apDivider.Text = "— Auto Play Version —"
apDivider.TextColor3 = Color3.fromRGB(0, 188, 255); apDivider.Font = Enum.Font.GothamBold; apDivider.TextSize = 9

local helperV1Btn = Instance.new("TextButton", speedMain)
helperV1Btn.Size = UDim2.new(0, 74, 0, 26); helperV1Btn.Position = UDim2.new(0, 8, 0, 193)
helperV1Btn.BackgroundColor3 = Color3.fromRGB(0, 160, 80); helperV1Btn.Text = "v1 ✔"
helperV1Btn.TextColor3 = Color3.new(1,1,1); helperV1Btn.Font = Enum.Font.GothamBold; helperV1Btn.TextSize = 11; helperV1Btn.BorderSizePixel = 0
Instance.new("UICorner", helperV1Btn).CornerRadius = UDim.new(0, 6)
Instance.new("UIStroke", helperV1Btn).Color = Color3.fromRGB(0, 220, 255)

local helperV2Btn = Instance.new("TextButton", speedMain)
helperV2Btn.Size = UDim2.new(0, 74, 0, 26); helperV2Btn.Position = UDim2.new(0, 90, 0, 193)
helperV2Btn.BackgroundColor3 = Color3.fromRGB(0, 120, 255); helperV2Btn.Text = "v2"
helperV2Btn.TextColor3 = Color3.new(1,1,1); helperV2Btn.Font = Enum.Font.GothamBold; helperV2Btn.TextSize = 11; helperV2Btn.BorderSizePixel = 0
Instance.new("UICorner", helperV2Btn).CornerRadius = UDim.new(0, 6)
Instance.new("UIStroke", helperV2Btn).Color = Color3.fromRGB(0, 220, 255)

local tpDivider = Instance.new("TextLabel", speedMain)
tpDivider.Size = UDim2.new(1, -20, 0, 18); tpDivider.Position = UDim2.new(0, 10, 0, 228)
...
duelsTpBtn.Size = UDim2.new(0, 160, 0, 30); duelsTpBtn.Position = UDim2.new(0, 10, 0, 250)
duelsTpBtn.BackgroundColor3 = Color3.fromRGB(0, 15, 50); duelsTpBtn.Text = "Enable Duels TP"
duelsTpBtn.TextColor3 = Color3.new(1, 1, 1); duelsTpBtn.Font = Enum.Font.GothamBold; duelsTpBtn.TextSize = 11; duelsTpBtn.BorderSizePixel = 0
...
-- Helper UI size slider
local helperSliderFrame, setHelperSlider = makeScaleSlider(speedMain, 0, 0.5, function(scale)
    helperScale.Scale = scale
end)
helperSliderFrame.Position = UDim2.new(0, 10, 0, 288)
helperSliderFrame.Size = UDim2.new(1, -20, 0, 32)

-- === AUTO PLAY VERSION SYNC (Helper <-> Standalone) ===
local function setApVisualAll(v1Active)
    if v1Active then
        helperV1Btn.BackgroundColor3 = Color3.fromRGB(0, 160, 80);  helperV1Btn.Text = "v1 ✔"
        helperV2Btn.BackgroundColor3 = Color3.fromRGB(0, 120, 255); helperV2Btn.Text = "v2"
    else
        helperV2Btn.BackgroundColor3 = Color3.fromRGB(0, 160, 80);  helperV2Btn.Text = "v2 ✔"
        helperV1Btn.BackgroundColor3 = Color3.fromRGB(0, 120, 255); helperV1Btn.Text = "v1"
    end
end

local function switchToV1()
    autoPlayVersion = 1; patrolMode = "none"
    cancelAutoPlays(); setApVisualAll(true)
end

local function switchToV2()
    autoPlayVersion = 2; auto1 = false; auto2 = false
    cancelAutoPlays(); setApVisualAll(false)
end

helperV1Btn.MouseButton1Click:Connect(function() if autoPlayVersion ~= 1 then switchToV1() end end)
helperV2Btn.MouseButton1Click:Connect(function() if autoPlayVersion ~= 2 then switchToV2() end end)
...
local function saveConfig()
    if writefile then
        ...
        -- Save slider values
        uiStates["MainUIScale"]   = mainScale.Scale
        uiStates["HelperUIScale"] = helperScale.Scale
        uiStates["SideUIScale"]   = sideScale.Scale
        writefile(configFile, HttpService:JSONEncode(uiStates))
        -- Save mobile positions (positions are saved separately on move)
        saveMobilePositions()
    end
end
...
local function loadConfig()
    if isfile and isfile(configFile) then
        ...
        if ok and type(data) == "table" then
            ...
            -- Restore slider scales
            if data["MainUIScale"] then
                mainScale.Scale = data["MainUIScale"]
                setMainSlider((data["MainUIScale"] - 0.5))
            end
            if data["HelperUIScale"] then
                helperScale.Scale = data["HelperUIScale"]
                setHelperSlider((data["HelperUIScale"] - 0.5))
            end
            if data["SideUIScale"] then
                sideScale.Scale = data["SideUIScale"]
                setSideSizeSlider((data["SideUIScale"] - 0.5))
            end
        end
    end
end
...
-- === ASSER AUTO PLAY VERSION UI (standalone, kept for compatibility) ===
...
local function setApVisual(v1Active)
    ...
    setApVisualAll(v1Active)
end

apV1Btn.MouseButton1Click:Connect(function() if autoPlayVersion ~= 1 then switchToV1() setApVisual(true) end end)
apV2Btn.MouseButton1Click:Connect(function() if autoPlayVersion ~= 2 then switchToV2() setApVisual(false) end end)
